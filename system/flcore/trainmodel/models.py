...
# Note: batch_size is a training parameter and should not be defined as a global network constant
...
class Net(nn.Module):
    def __init__(self, num_classes=10):
        ...
        self.conv1 = nn.Conv2d(1, 10, 2, 1)  # Fixed: use explicit channel number instead of batch_size
        self.conv2 = nn.Conv2d(10, 32, 2, 1)
...
class CifarNet(nn.Module):
    def __init__(self, num_classes=10):
        ...
        self.conv2 = nn.Conv2d(6, 16, 5)  # Fixed: use 16 channels (standard LeNet config) instead of batch_size
    ...
    def forward(self, x):
        ...
        x = x.view(-1, 16 * 5 * 5)  # Fixed: use 16 to match conv2 output channels
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        x = self.fc(x)
        x = F.log_softmax(x, dim=1)
